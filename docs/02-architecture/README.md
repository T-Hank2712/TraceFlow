...
- Control plane quản lý user, workspace, project, application, API key và project-scoped search.
...
- OpenSearch là nơi lưu log phục vụ search, trace lookup và operational insights nếu bật.
- Redis là supporting infrastructure cho validation cache, rate limiting và counter ngắn hạn.
- Multi-tenancy là yêu cầu kiến trúc, không phải chi tiết phụ.
- Failure handling, backpressure và observability tối thiểu phải được thiết kế từ đầu.
...
- Khi thêm retry, DLQ, rate limiting, retention hoặc extension worker.
